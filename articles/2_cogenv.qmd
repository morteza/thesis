...
Figure 1: Overall architecture of CogEnv. CogEnv communicates with AndroidEnv via Protocol Buffer messages and manages access to the Behaverse events. $O_t$ is the screenshot of the task at time t, $O^{\prime}_t$ is the extra observations extracted from the Behaverse events including information about the task and stimuli, $r_t$ is the reward, and $A_t$ is the agent's action.

### Tasks
CogEnv currently runs four Behaverse tasks (see Figure 2) selected to cover main components of cognitive control (see Chapter 1). In the Belval Matrices test for example, agents are shown a matrix of symbols on a 3x3 grid, where one of the cells of the matrix has been removed, and they are tasked to identify the missing cell from a set of eight options (panel D of Figure 2). The Belval Matrices can randomly generate a large number of test items of varying difficulty and structure, which makes this test interesting for human and artificial learning studies.



Figure 2: Screenshot ($O_t$) of four Behaverse tasks. A) Digit Span (working memory), B) N-Back (working memory), C) Trail Making Test (cognitive flexibility), and D) Belval Matrices (matrix reasoning). See \href{https://behaverse.org}{behaverse.org}.
...
We collected data (accuracy and response time) from 200 human participants completing 20 items of the Belval Matrices (see Figure 3) and are currently training a selection of discrete control agents on the same test  \cite<i.e., DQN and R2D2 from the Acme Tensorflow library; see>{hoffman2020, toyama2021}: agents are trained on 1000 randomly generated items and tested on a set of 20 unseen test items, the exact same used with human participants.
...
Figure 3: Hypothetical scenarios when comparing the performance of humans versus computational agents (see text).
